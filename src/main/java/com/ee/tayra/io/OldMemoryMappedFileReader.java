...
public class OldMemoryMappedFileReader implements DocumentReader {
  ...
  public OldMemoryMappedFileReader(final String fileName) {
  ...
  final void initializeBuffer(final String toAppend) {
    index = 0;
    try {
  ...
  public final String[] getBuffer() throws IOException {
    if (readingFromStart) {
      sourceFile.seek(0L);
    }
